...
func (current *User) ClaimPacket(ctx context.Context, packetId string) (*Packet, error) {
	...
	err = session.Database(ctx).RunInTransaction(ctx, &sql.TxOptions{Isolation: sql.LevelSerializable}, func(ctx context.Context, tx *sql.Tx) error {
		...
		if err == sql.ErrNoRows {
			...
			if err == nil && !b {
				dm, err := createDistributeMessage(ctx, bot.UuidNewV4().String(), bot.UuidNewV4().String(), "", config.AppConfig.Mixin.ClientId, packet.UserId, MessageCategoryPlainText, base64.StdEncoding.EncodeToString([]byte(fmt.Sprintf(config.AppConfig.MessageTemplate.GroupOpenedRedPacket, current.FullName))), false)
				if err != nil {
					return err
				}
